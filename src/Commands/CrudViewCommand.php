...
class CrudViewCommand extends Command
{
    ...
    protected $signature = 'crud:view
                            ...
                            {--filters= : The name of the filters key.}
    ...
    public function handle()
    {
        ...
        $this->buildFilterFieldsHtml($filters);
    ...
    public function templateIndexVars($newIndexFile)
    {
        ...
        File::put($newIndexFile, str_replace('%%filterFieldsHtml%%', $this->filterFieldsHtml, File::get($newIndexFile)));
...
EOD;
    ...
    public function getFilterFieldsHtmlWrapper()
    {
        $html =
            <<<EOD
        <div class="panel panel-default">
            <div class="panel-body">
                <form class="form-inline" action="/%%routeGroup%%%%viewName%%" method="GET">
                    %%filterFields%%

                    <button type="submit" class="btn btn-default">
                        Search</button>
                    <a class="btn btn-default" href="/%%routeGroup%%%%viewName%%">
...
EOD;
     ...
     * @return string
     */
    public function getFilterFieldWrapper ()
    {
        return '<div class="form-group">
                  <label class="control-label" for="%%inputId%%">%%inputLabel%%</label>
                  <input type="text" name="filters[%%inputId%%]" class="form-control" id="%%inputId%%" placeholder="%%inputLabel%%">
                </div>
        ';
    }
     ...
     * @param $filters
     */
    private function buildFilterFieldsHtml($filters)
    {
        if (!$filters) {
        ...
        foreach ($filterParts as $filter) {
            $itemArr = explode('#', $filter);
            $field = $itemArr[0];
            $label = ucwords(str_replace('_', ' ', $field));
            $tmp = $this->getFilterFieldWrapper();

            $tmp = str_replace(['%%inputId%%', '%%inputLabel%%'], [$field, $label], $tmp);

            $fieldsHtml .= $tmp;
        }

        $this->filterFieldsHtml = str_replace(
            ['%%routeGroup%%', '%%filterFields%%'],
            [$this->routeGroup, $fieldsHtml],
            $this->getFilterFieldsHtmlWrapper()
        );
    }
}
